...
class Vector:
    ...
    def __mul__(self, other: Union[int, float, complex, str, 'Vector']) -> Union[int, float, complex, 'Vector']:
        """
        If `other` can be cast to number then return new vector which represent multiplication vector `self`
        to scalar `other`.
...
class Vector3D(Vector):
    ...
    def cross_product(self, other):
        if not isinstance(other, Vector3D):
            raise TypeError("Unexpected argument type: {}. Vector3D expected".format(type(other)))
        else:
...
class TestVector3D(unittest.TestCase):
    ...
    def test_cross_product(self):
        ...
        self.assertEqual(vector1.cross_product(vector2), Vector3D(0, 0, 0))
        with self.assertRaises(TypeError):
            vector1.cross_product(Vector(1, 2, 3, 4))
